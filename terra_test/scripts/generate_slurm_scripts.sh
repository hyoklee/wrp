#!/bin/bash
#
# Generate SLURM scripts for all 16 TERRA benchmark configurations
#
...
for CONFIG in "${CONFIGS[@]}"; do
    ...
    SCRIPT_FILE="${SCRIPT_DIR}/slurm_terra_${NODES}n_${TASKS}t.sh"

    cat > "$SCRIPT_FILE" << 'SLURM_HEADER'
#!/bin/bash
SLURM_HEADER

    cat >> "$SCRIPT_FILE" << SLURM_BODY
#SBATCH --job-name=terra_${NODES}n_${TASKS}t
#SBATCH --nodes=${NODES}
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=${TASKS}
#SBATCH --time=01:00:00
#SBATCH --output=${TEST_DIR}/logs/terra_${NODES}n_${TASKS}t_%j.out
#SBATCH --error=${TEST_DIR}/logs/terra_${NODES}n_${TASKS}t_%j.err
#SBATCH --exclusive
...
echo ""
echo "All SLURM scripts generated in: ${SCRIPT_DIR}"
