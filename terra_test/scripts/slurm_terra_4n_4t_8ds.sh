#!/bin/bash
#SBATCH --job-name=terra_4n_4t_8ds
#SBATCH --nodes=4
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=4
#SBATCH --time=00:30:00
#SBATCH --output=/mnt/common/hyoklee/core.iowarp/terra_test/logs/terra_4n_4t_8ds_%j.out
#SBATCH --error=/mnt/common/hyoklee/core.iowarp/terra_test/logs/terra_4n_4t_8ds_%j.err
#SBATCH --exclusive

set -e

# Configuration
SCRIPT_DIR="/mnt/common/hyoklee/core.iowarp/terra_test/scripts"
TEST_DIR="/mnt/common/hyoklee/core.iowarp/terra_test"
CORE_DIR="/mnt/common/hyoklee/core.iowarp"
BUILD_DIR="${CORE_DIR}/build"
NODES=4
TASKS=4

# Activate conda environment
source /home/hyoklee/mc3/etc/profile.d/conda.sh
conda activate iowarp-build

# Set library paths
export LD_LIBRARY_PATH="${BUILD_DIR}/bin:${LD_LIBRARY_PATH}"

# Create logs directory
mkdir -p "${TEST_DIR}/logs"

echo "=========================================="
echo "TERRA Benchmark (8 datasets): ${NODES} nodes, ${TASKS} tasks"
echo "Job ID: ${SLURM_JOB_ID}"
echo "Nodes: $(scontrol show hostnames ${SLURM_JOB_NODELIST})"
echo "=========================================="

# Generate hostfile for distributed run with IP addresses
HOSTFILE="${TEST_DIR}/configs/hostfile_${SLURM_JOB_ID}"
echo "Generating hostfile with IP addresses..."
> "${HOSTFILE}"  # Clear/create hostfile
for hostname in $(scontrol show hostnames "${SLURM_JOB_NODELIST}"); do
    # Resolve hostname to IP address
    ip_addr=$(getent hosts "${hostname}" | awk '{print $1}' | head -1)
    if [[ -z "${ip_addr}" ]]; then
        # Fallback: try using the hostname directly
        ip_addr="${hostname}"
    fi
    echo "${ip_addr}" >> "${HOSTFILE}"
    echo "  ${hostname} -> ${ip_addr}"
done
echo "Hostfile contents:"
cat "${HOSTFILE}"

# Export hostfile path for all processes
export CHI_HOSTFILE="${HOSTFILE}"

# Create temporary runtime config with actual values
CONFIG_FILE="${TEST_DIR}/configs/terra_runtime_config.yaml"
RUNTIME_CONFIG_TEMP="${TEST_DIR}/configs/runtime_${SLURM_JOB_ID}.yaml"

# Substitute environment variables in config
sed -e "s/\${CHI_SCHED_THREADS:-8}/${TASKS}/g" \
    -e "s/\${CHI_PORT:-5555}/5555/g" \
    -e "s/\${CHI_NEIGHBORHOOD:-32}/${NODES}/g" \
    -e "s/\${CHI_NEIGHBORHOOD:-1}/${NODES}/g" \
    -e "s|\${CHI_HOSTFILE:-}|${HOSTFILE}|g" \
    "${CONFIG_FILE}" > "${RUNTIME_CONFIG_TEMP}"

export CHI_SERVER_CONF="${RUNTIME_CONFIG_TEMP}"

echo ""
echo "Runtime config: ${RUNTIME_CONFIG_TEMP}"
echo "Networking section:"
grep -A5 "networking:" "${RUNTIME_CONFIG_TEMP}"

# Results file - use 8 datasets config
RESULT_FILE="${TEST_DIR}/results/terra_${NODES}n_${TASKS}t_8ds_${SLURM_JOB_ID}.log"
OMNI_FILE="${TEST_DIR}/configs/terra_omni_8datasets.yaml"

# Data size estimate (MB) - smaller for 8 datasets
TOTAL_DATA_SIZE_MB=663

echo ""
echo "Step 1: Starting Chimaera runtime on all nodes..."

# Start runtime on all nodes
if [[ ${NODES} -gt 1 ]]; then
    srun --nodes=${NODES} --ntasks=${NODES} \
        "${BUILD_DIR}/bin/chimaera_start_runtime" &
else
    "${BUILD_DIR}/bin/chimaera_start_runtime" &
fi

RUNTIME_PID=$!
echo "  Waiting for distributed runtime initialization (30 seconds)..."
sleep 30  # Wait for distributed runtime initialization across all nodes

echo "  Runtime started (PID: ${RUNTIME_PID})"

echo ""
echo "Step 2: Running HDF5 assimilation benchmark (8 datasets)..."

# Record start time
START_TIME=$(date +%s.%N)

# Run the OMNI processor (CTE handles distributed task routing to all nodes)
"${BUILD_DIR}/bin/wrp_cae_omni" "${OMNI_FILE}" 2>&1 | tee "${TEST_DIR}/logs/omni_${NODES}n_${TASKS}t_8ds_${SLURM_JOB_ID}.log"
OMNI_EXIT=${PIPESTATUS[0]}

# Record end time
END_TIME=$(date +%s.%N)

# Calculate elapsed time
ELAPSED_TIME=$(echo "${END_TIME} - ${START_TIME}" | bc)

echo ""
echo "Step 3: Calculating results..."

# Calculate bandwidth
BANDWIDTH_MBps=$(echo "scale=2; ${TOTAL_DATA_SIZE_MB} / ${ELAPSED_TIME}" | bc)

# Write results
{
    echo "=========================================="
    echo "TERRA Fusion I/O Benchmark Results (8 datasets)"
    echo "=========================================="
    echo ""
    echo "Configuration:"
    echo "  Date: $(date)"
    echo "  Job ID: ${SLURM_JOB_ID}"
    echo "  Nodes: ${NODES}"
    echo "  Tasks: ${TASKS}"
    echo "  Total Tasks: $((NODES * TASKS))"
    echo "  Datasets: 8"
    echo ""
    echo "Test Results:"
    echo "  OMNI Exit Code: ${OMNI_EXIT}"
    echo "  Elapsed Time: ${ELAPSED_TIME} seconds"
    echo "  Data Size: ${TOTAL_DATA_SIZE_MB} MB"
    echo "  Bandwidth: ${BANDWIDTH_MBps} MB/s"
    echo ""
} | tee "${RESULT_FILE}"

echo "Step 4: Stopping runtime..."

# Stop the runtime
kill ${RUNTIME_PID} 2>/dev/null || true
wait ${RUNTIME_PID} 2>/dev/null || true

# Cleanup
rm -f "${RUNTIME_CONFIG_TEMP}"
rm -f "${HOSTFILE}"

echo "  Runtime stopped"
echo ""
echo "=========================================="
echo "Benchmark Complete (8 datasets)"
echo "=========================================="

exit ${OMNI_EXIT}
